...
+ (NSString *)replaceString:(NSString *)str replacekv:(NSDictionary *)kv
{
    ...
    [s setString:str];
    NSArray *ar = [[kv allKeys] sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
...
{
    return [NSString replaceString:self replacekv:kv];
}
...
//        if (pinYinArray==nil) {
//            pinYinArray=[[NSMutableArray new]autorelease];
//        }
//        [pinYinArray addObject:strRes];
//        i++;
//
//    }
    return [self getPinYinFromChiniseString:string abortUnCNString:YES];
}
...
- (NSMutableArray *)getPinYinFromChiniseString:(NSString *)string abortUnCNString:(BOOL)bAbort
{
    ...
    for (int i = 0; i < iLen; i++)
    {
        if ((unsigned char) gb2312_string[i] < 0x80)
        {
            ...
            {
                if (pinYinArray == nil)
                    pinYinArray = [[NSMutableArray new] autorelease_mm];
                char s[2] = {gb2312_string[i], '\0'};
                [pinYinArray addObject:[NSString stringWithCString:s encoding:enc]];
                continue;
            }
            else
                return nil;
...
+ (NSMutableArray *)getPinYinFromChiniseString:(NSString *)string abortUnCNString:(BOOL)bAbort
{
    return [@"" getPinYinFromChiniseString:string abortUnCNString:bAbort];
}
 */
/*
...
{
    NSMutableDictionary *sort = [[NSMutableDictionary new] autorelease_mm];
    ...
    for (int i = 0; i < arEntcontact.count; i++)
    {
        ...
        if (szName.length > 0)
        {
            ...
            else if (YES == [szName isEnglishString])
            {
                ...
                arPinyinname = [[NSMutableArray new] autorelease_mm];
...
+ (NSArray *)sortByPinYin:(NSArray *)arEntcontact comparePinYinName:(NSArray *(^)(id obj))compare
{
    ...
    for (int i = 0; i < arEntcontact.count; i++)
    {
        NSArray *arPinyinname = compare([arEntcontact objectAtIndex:i]);
        if (ar.count > 0)
        {
            //通过szName拼音比较（按升序排序），插入contact
            for (int j = 0; j < ar.count; j++)
            {
                NSArray *arCompare = compare([ar objectAtIndex:j]);
                int index = 0;
                if (arPinyinname.count > arCompare.count)
                {
...
+ (NSString *)megerStringWithTag:(NSArray *)arStr tag:(NSString *)szTag
{
    if (arStr.count <= 0)
        return @"";
    NSMutableString *s = [NSMutableString new];
    for (NSString *ss in arStr)
    {
        [s appendFormat:@"%@%@", ss, szTag];
    }
    [s deleteCharactersInRange:NSMakeRange(s.length - szTag.length, szTag.length)];
    return s;
}
...
+ (NSString *)megerStringWithTag:(NSArray *)arStr property:(NSString *)szProperty tag:(NSString *)szTag
{
    ...
    for (id ss in arStr)
    {
        [s appendFormat:@"%@%@", [ss valueForKey:szProperty], szTag];
    }
    [s deleteCharactersInRange:NSMakeRange(s.length - szTag.length, szTag.length)];
    return s;
}
...
{
    NSScanner *scanner = [[NSScanner alloc] initWithString:arStr];
...
{
    NSArray *ar = [str componentsSeparatedByString:tag];
    if(ar.count%2!=0)
        return nil;
    XMSQueueDictionary *qd = [[XMSQueueDictionary alloc]init];
    for(int i =0;i<ar.count;i++,i++)
        [qd addKvPairDirect:[XMSKvPair kvPairWithKey:[ar objectAtIndex:i] value:[ar objectAtIndex:i+1]]];
    return qd;
}

- (XMSQueueDictionary *)splitKeyValueStringWithTag:(NSString*)tag
{
    return [NSString splitKeyValueString:self withTag:tag];
}
...
+ (XMSQueueDictionary *)splitString:(NSString*)str withStringTag:(NSString*)tag1 andKeyValueStringTag:(NSString*)tag2
{
    ...
    NSArray *ar = [str componentsSeparatedByString:tag1];
    for (NSString *s in ar)
    {
        [qd addKvPairFromOtherQueueDictionary:[NSString splitKeyValueString:s withTag:tag2]];
    }
    return qd;
}*/
...
{
    return [NSString splitString:self withStringTag:tag1 andKeyValueStringTag:tag2];
}

+ (CGSize)charSizeWithFont:(UIFont *)font
{
#if __IPHONE_OS_VERSION_MIN_REQUIRED <=  __IPHONE_6_1
    return [@"国" sizeWithFont:font];
#else
    NSDictionary *attributes = @{NSFontAttributeName:font};
    CGSize size = [@"国" sizeWithAttributes:attributes];
...
+ (CGSize)charSizeWithSystemFontSize:(CGFloat)fontSize
{
    ...
    return [NSString charSizeWithFont:font];
}
...
{
    UIFont *font = [UIFont systemFontOfSize:fontSize];
    return [self stringSizeWithFont:font];
}

- (CGSize)stringSizeWithFont:(UIFont*)font forWidth:(CGFloat)width
{
    return [self stringSizeWithFont:font forConstrainSize:CGSizeMake(width, INT_MAX)];
}

- (CGSize)stringSizeWithSystemFontSize:(CGFloat)fontSize forWidth:(CGFloat)width
{
    UIFont *font = [UIFont systemFontOfSize:fontSize];
    return [self stringSizeWithFont:font forWidth:width];
}

- (CGSize)stringSizeWithFont:(UIFont*)font forConstrainSize:(CGSize)constrainSize
{
#if __IPHONE_OS_VERSION_MIN_REQUIRED <=  __IPHONE_6_1
    return [self sizeWithFont:font constrainedToSize:constrainSize];
#else
    NSDictionary *attributes = @{NSFontAttributeName:font};
    CGSize size = [self boundingRectWithSize:constrainSize options:NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesFontLeading attributes:attributes context:nil].size;
    return CGSizeMake(MIN(ceilf(size.width),constrainSize.width), ceilf(size.height));
...
{
    UIFont *font = [UIFont systemFontOfSize:fontSize];
    return [self stringSizeWithFont:font forConstrainSize:constrainSize];
}

- (CGSize)wrapStringSizeWithFont:(UIFont*)font forWidth:(CGFloat)width
{
//#if __IPHONE_OS_VERSION_MIN_REQUIRED <=  __IPHONE_6_1
//    return [self sizeWithFont:font constrainedToSize:CGSizeMake(width, INT_MAX) lineBreakMode:NSLineBreakByWordWrapping];
//#else
//    NSDictionary *attributes = @{NSFontAttributeName:font};
//    CGSize size = [self boundingRectWithSize:CGSizeMake(width,INT_MAX) options:NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attributes context:nil].size;
//    return CGSizeMake(ceilf(size.width), ceilf(size.height));
//#endif
    return [self wrapStringSizeWithFont:font forConstrainSize:CGSizeMake(width, INT_MAX)];
}

- (CGSize)wrapStringSizeWithSystemFontSize:(CGFloat)fontSize forWidth:(CGFloat)width
{
    UIFont* font = [UIFont systemFontOfSize:fontSize];
    return [self wrapStringSizeWithFont:font forWidth:width];
}

- (CGSize)wrapStringSizeWithFont:(UIFont*)font forConstrainSize:(CGSize)constrainSize
{
#if __IPHONE_OS_VERSION_MIN_REQUIRED <=  __IPHONE_6_1
    return [self sizeWithFont:font constrainedToSize:constrainSize lineBreakMode:NSLineBreakByWordWrapping];
#else
    NSDictionary *attributes = @{NSFontAttributeName:font};
    CGSize size = [self boundingRectWithSize:constrainSize options:NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attributes context:nil].size;
    return CGSizeMake(MIN(ceilf(size.width),constrainSize.width), ceilf(size.height));
#endif
}
...
{
    UIFont* font = [UIFont systemFontOfSize:fontSize];
    return [self wrapStringSizeWithFont:font forConstrainSize:constrainSize];
}
...
- (NSRange)truncatesStringOfLines:(NSUInteger)lines forContainWidth:(CGFloat)containWidth font:(UIFont*)font
{
    ...
    {
        CGSize singleSize = [NSString charSizeWithFont:font];
        CGFloat singleHeight = singleSize.height;
        CGFloat totalHeight = [self wrapStringSizeWithFont:font forWidth:containWidth].height;
        NSUInteger l = ceil(totalHeight / singleHeight);
        if(l <= lines)
            return NSMakeRange(0, self.length);
        
        NSUInteger i = self.length / 2;
        for(;;)
        {
            NSString *tmp = [self substringWithRange:NSMakeRange(0, i)];
            CGFloat totalHeight = [tmp wrapStringSizeWithFont:font forWidth:containWidth].height;
            l = ceil(totalHeight / singleHeight);
            if(l > lines)
            {
                i = i / 2;
        ...
        NSUInteger testLen = containWidth / singleSize.width;
        for(;;)
        {
            if(i + testLen > self.length)
                testLen = self.length - i;
            NSString *tmp = [self substringWithRange:NSMakeRange(0, i + testLen)];
            CGFloat totalHeight = [tmp wrapStringSizeWithFont:font forWidth:containWidth].height;
            l = ceil(totalHeight / singleHeight);
            if(l > lines)
            {
                ...
                if(testLen <= 0)
                {
//                    NSString *ddd= [self substringWithRange:NSMakeRange(0, i)];
//                    
//                    CGSize ss = [[self substringWithRange:NSMakeRange(0, i)] wrapStringSizeWithFont:font forWidth:containWidth];
//                    
//                    int a =0 ;                    
...
//- (BOOL)isFaxNumber
//{
//    return [NSString isFaxNumber:self];
//}
...
- (NSString *)base64{
    NSData *inputData = [self dataUsingEncoding:NSUTF8StringEncoding];
    NSString *authorization = [inputData base64EncodedStringWithOptions:0];
...
//- (GUID)GUIDValue
//{
//    return [XMSGUID GUIDBytesWithGUIDString:self];
//}
